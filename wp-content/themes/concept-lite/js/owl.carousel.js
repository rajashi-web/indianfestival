			...
			'initialized.owl.carousel change.owl.carousel': jQuery.proxy(function(e) {
				...
				if ((e.property && e.property.name === 'position') || e.type === 'initialized') {
					...
					while (iterator--) {
						...
						clones && jQuery.each(this._core.clones(positions[iterator]), load);
					}
				}
			}, this)
		};
	...
	Lazy.prototype.load = function(position) {
		...
		jQueryelements.each(jQuery.proxy(function(index, element) {
			...
			if (jQueryelement.is('img')) {
				...
				jQuery(new Image()).one('load.owl.lazy', jQuery.proxy(function() {
					jQueryelement.css({
...
;(function(jQuery, window, document, undefined) {
	...
	Navigation.prototype.initialize = function() {
		...
		this._controls.jQueryindicators = options.dotsContainer ? jQuery(options.dotsContainer)
			: jQuery('<div>').hide().addClass(options.dotsClass).appendTo(this._controls.jQuerycontainer);
...
jQuery(document).ready(function(){
  jQuery(".owl-carousel").owlCarousel({items:1, loop:true, dots: false, autoplay:true, smartSpeed: 1000, nav:true, navText:['&#10148;','&#10148;']});
});
