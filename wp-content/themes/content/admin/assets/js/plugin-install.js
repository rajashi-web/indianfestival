jQuery(document).ready(function($) {
    'use strict';
    var this_obj = content_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'content_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'content_hide_customizer_notice';
            sdata.chilly_plugin_slug = slug;
        }else{
            sdata.action = 'content_hide_customizer_companion_notice';
        }
