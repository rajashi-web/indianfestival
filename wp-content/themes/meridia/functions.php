...
// Constants
define( 'MERIDIA_THEME_VERSION', '2.2.0' );
...
require MERIDIA_THEME_DIR . '/includes/customizer/customizer.php';
/**
* Theme styles
*/
function meridia_theme_styles()
{
    ...
    wp_enqueue_style(
        'meridia-styles',
        MERIDIA_THEME_URI . '/style.min.css',
        array( 'bootstrap', 'meridia-font-icons' ),
        MERIDIA_THEME_VERSION,
        'all'
    );
    // Dynamic CSS for custom breakpoints
    $dynamic_styles = meridia_dynamic_styles();
    
    if ( get_template_directory() === get_stylesheet_directory() ) {
        wp_add_inline_style( 'meridia-styles', $dynamic_styles );
    } else {
        wp_add_inline_style( 'meridia-parent-styles', $dynamic_styles );
    }
...
{
    wp_enqueue_style( 'meridia-gutenberg-editor-styles', get_theme_file_uri( '/assets/css/gutenberg-editor-style.min.css' ), false );
    if ( !class_exists( 'Kirki' ) ) {
...
function meridia_theme_js()
{
    ...
    // PHP to JS
    global  $wp_query ;
    $layout = get_theme_mod( 'meridia_post_layout_type', 'grid' );
    $columns = get_theme_mod( 'meridia_homepage_columns', 'col-lg-6' );
    $breakpoint_lg_up = get_theme_mod( 'meridia_header_navbar_mobile_collapse_breakpoint', 992 );
    $breakpoint_lg_down = $breakpoint_lg_up - 1;
    // Load more
    wp_localize_script( 'meridia-scripts', 'meridia_php_params', array(
        ...
        'breakpoint_lg_up'   => esc_html( $breakpoint_lg_up ),
        'breakpoint_lg_down' => esc_html( $breakpoint_lg_down ),
    ) );
}
...
{
    wp_enqueue_style( 'meridia-admin-styles', MERIDIA_THEME_URI . '/assets/admin/css/meridia-admin-styles.css' );
}
