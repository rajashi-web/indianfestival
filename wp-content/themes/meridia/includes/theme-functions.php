...
// Replace escerpt dots
function meridia_excerpt_more($more) {
	return '...';
}
add_filter('excerpt_more', 'meridia_excerpt_more', 21 );
		...
		while ( $query->have_posts() ): $query->the_post();
			...
			switch ( $layout ) {
				...
				case 'grid-first-large' : ?>
					...
					<?php get_template_part( 'template-parts/posts/grid-post-small' ); ?>
					</div>
					<?php
					break;
				...
				case 'horizontal-cards' :
					get_template_part( 'template-parts/posts/pro/horizontal-cards-post' );
					break;

				case 'vertical-cards' : ?>
					<div class="<?php echo esc_attr( $columns ); ?> col-sm-6">
					<?php get_template_part( 'template-parts/posts/pro/vertical-cards-post' ); ?>
					</div>
					<?php
					break;

				default :
					get_template_part( 'template-parts/posts/grid-post-small' );
		...
		if ( get_the_author_meta( 'description' ) ) : ?>
			<div class="entry-author entry-author--box clearfix">
				...
				<div class="entry-author__info">
					...
					<?php endif; ?>
					<p class="entry-author__description mb-0"><?php the_author_meta( 'description' ); ?></p>
...
if ( ! function_exists( 'meridia_get_post_thumb_size' ) ) {
	/**
	 * Check the columns and return proper thumb size
	 * @return string $thumb_size post thumb size
	 */
	function meridia_get_post_thumb_size() {
		$columns = get_theme_mod( 'meridia_homepage_columns', 'col-lg-6' );
		$thumb_size = '';

		switch ( $columns ) {
			case 'col-lg-12':
				$thumb_size = 'meridia_extra_large';
				break;

			case 'col-lg-6':
				$thumb_size = 'meridia_large';
				break;
			
			default:
				$thumb_size = 'meridia_medium';
				break;
		}
...
if ( ! function_exists( 'meridia_post_layout_type' ) ) {
	/**
	 * Check post layout type based on customizer and meta settings
	 * @return string $layout post layout type
	 */
	function meridia_post_layout_type( $default = 'grid' ) {
		$layout = '';

		switch ( get_theme_mod( 'meridia_post_layout_type', $default ) ) {
...
if ( ! function_exists( 'meridia_layout_type' ) ) {
	 ...
	 */
	function meridia_layout_type( $type, $default = 'right-sidebar' ) {
		$layout = '';
		$meta = get_post_meta( get_the_ID(), 'meridia_content_layout', true );
		...
		if ( is_archive() || is_404() || is_search() || is_front_page() ) {
			$meta = '';
		}

		if ( 'left-sidebar' == get_theme_mod( 'meridia_' . $type . '_layout_type', $default ) ) {
			$layout = ( $meta ) ? $meta : 'left-sidebar';		
		}

		if ( 'right-sidebar' == get_theme_mod( 'meridia_' . $type . '_layout_type', $default ) ) {
			$layout = ( $meta ) ? $meta : 'right-sidebar';
		}

		if ( 'fullwidth' == get_theme_mod( 'meridia_' . $type . '_layout_type', $default ) ) {			
...
if ( ! function_exists( 'meridia_single_post_layout_type' ) ) {
	/**
	 * Check single post style based on customizer and meta settings
	 * @return string $layout Layout type
	 */
	function meridia_single_post_layout_type() {
		$layout = '';
		$meta = get_post_meta( get_the_ID(), 'meridia_single_post_style', true );
		$page_for_posts = get_option( 'page_for_posts' );
		...
		if ( 'single-post-style-1' == get_theme_mod( 'meridia_single_post_style', 'single-post-style-1' ) ) {
			$layout = ( $meta ) ? $meta : 'single-post-style-1';		
		}

		if ( 'single-post-style-2' == get_theme_mod( 'meridia_single_post_style', 'single-post-style-1' ) ) {
			$layout = ( $meta ) ? $meta : 'single-post-style-2';		
		}
...
if ( ! function_exists( 'meridia_page_title_layout_type' ) ) {
	/**
	 * Check page title style based on customizer and meta settings
	 * @return string $layout Layout type
	 */
	function meridia_page_title_layout_type() {
		$layout = '';
		$meta = get_post_meta( get_the_ID(), 'meridia_page_title_layout_type', true );
		$page_for_posts = get_option( 'page_for_posts' );
		...
		if ( 'page-title-style-1' == get_theme_mod( 'meridia_page_title_layout_type', 'page-title-style-1' ) ) {
			$layout = ( $meta ) ? $meta : 'page-title-style-1';		
		}

		if ( 'page-title-style-2' == get_theme_mod( 'meridia_page_title_layout_type', 'page-title-style-1' ) ) {
			$layout = ( $meta ) ? $meta : 'page-title-style-2';		
		}
...
/**
 * Get featured posts, selected with metaboxes.
 *
 * @return array array of featured posts
 */
function meridia_get_featured_posts() {
...
function meridia_body_classes( $classes ) {
	...
	// Page layout class
	if ( is_page() ) {
		$classes[] = 'meridia-' . meridia_layout_type( 'page' );
	}

	// Homepage layout class
	if ( is_home() ) {
		if ( meridia_is_woocommerce_activated() && is_product() ) {
			$classes[] = '';
		} else {
			$classes[] = 'meridia-' . meridia_layout_type( 'homepage' );
		}
	}

	// Single post layout class
	if ( is_single() ) {
		$classes[] = 'meridia-' . meridia_layout_type( 'single_post' );

		$classes[] = 'meridia-' . meridia_single_post_layout_type();
	}

	// Shop Layout Class
	if ( meridia_is_woocommerce_activated() ) {
		if ( is_woocommerce() || is_shop() ) {
			$classes[] = 'meridia-' . meridia_layout_type( 'shop' );
		}
	}
	...
	if ( is_archive() ) {
		if ( meridia_is_woocommerce_activated() && is_shop() ) {
			...
			$classes[] = 'meridia-' . meridia_layout_type( 'archives' );
		}
	}

	// Search layout class
	if ( is_search() ) {
		$classes[] = 'meridia-' . meridia_layout_type( 'search_results' );
	}
...
/**
 * Get mobile brekapoint.
 *
 * @return array Array of breakpoints
 */
function meridia_get_mobile_breakpoint() {
	$breakpoint = get_theme_mod( 'meridia_header_navbar_mobile_collapse_breakpoint', 992 );

	$breakpoints = array(
		'lg_up' 	=> esc_html( $breakpoint ),
		'lg_down' => esc_html( $breakpoint - 1 )
	);
 ...
 * @return string
 */
function meridia_minify_css( $input ) {
