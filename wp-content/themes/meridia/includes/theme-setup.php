...
if ( !function_exists( 'meridia_theme_setup' ) ) {
    ...
    */
    function meridia_theme_setup()
    {
        ...
        // Remove admin notice for Instagram Feed
...
function meridia_widgets_init()
{
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Below Featured Area', 'meridia' ),
        'id'            => 'meridia-below-featured-area',
        'before_widget' => '<div id="%1$s" class="widget-below-featured-area %2$s">',
        'after_widget'  => '</div>',
 ...
 */
add_action( 'admin_init', 'remove_insta_alerts' );
function remove_insta_alerts()
{
    remove_action( 'admin_notices', 'sbi_notices_html', 8 );
 ...
 */
function meridia_freemius_extra_permissions( $permissions )
{
    $permissions['newsletter'] = array(
        'icon-class' => 'dashicons dashicons-email-alt',
        'label'      => meridia_fs()->get_text_inline( 'Newsletter', 'meridia' ),
        'desc'       => meridia_fs()->get_text_inline( 'Your email is added to our newsletter. Updates, announcements, marketing, no spam. Unsubscribe anytime.', 'meridia' ),
        'priority'   => 15,
    );
    return $permissions;
}

meridia_fs()->add_filter( 'permission_list', 'meridia_freemius_extra_permissions' );